...
export class MinecraftComponent implements OnInit, OnDestroy {
  state: boolean = false;
  ...
  loadUser(name: string) {
    ...
    this.http.get(this.ROOT_URL + "/information/minecraft?pseudo=" + name).subscribe(result => {
      var tmp = Object.values(result);
      var obj: user = {
        name: tmp[1],
        image: JSON.parse(atob(tmp[2][0].value)).textures.SKIN.url
      }
      this.users.push(obj);
    })
  }
