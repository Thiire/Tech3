...
export class LoremComponent implements OnInit, OnDestroy {
  state: boolean = false;
  ...
  loadText() {
    ...
    this.http.get(this.ROOT_URL + "/information/lorem").catch(err => {
      var tmp = Object.values(err);
      var obj: text = {
        description: tmp[7].text
      }
      this.texts.push(obj);
